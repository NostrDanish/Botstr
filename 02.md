...
```
Join material is the bundle's *membership* subset (CORD-05): `community_id, owner, owner_salt, community_root, root_epoch, control_pk, channels, relays, name`. Never the icon (a device folds it from the Control Plane) and never the link fields (expiry and attribution belong to the invite, not the membership).
